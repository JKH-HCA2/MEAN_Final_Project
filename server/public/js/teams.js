...
function getDataByLeague() {
    ...
    $("#dataTable").remove();
    buildTableFrame();
    $.getJSON("/teams/data/byleague/" + selected, teams => {
        $.each(teams, (index, team) => {
            getTableBody(team);
        });
        addRegBtn();
...
function getDataByTeam() {
    ...
    $("#dataTable").remove();
    buildTableFrame();
    $.getJSON("/teams/data/" + selected, team => {
        getTableBody(team);
    });
    addRegBtn();
}
    ...
    let tableFrame = `<table class='table table-responsive table-hover text-center table-light table-bordered mt-3' id='dataTable'>
                                <thead class='thead-dark' id='tableHead'>
                                    <tr>   
                                        ...
                                        <th class='align-middle'>League Name</th>
